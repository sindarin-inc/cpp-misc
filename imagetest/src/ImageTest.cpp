...
static auto readPNG(const char *path, int &width, int &height, std::vector<uint8_t> &out) -> bool {
    ...
    if (!png_ptr) {
        std::fclose(fp);
        fprintf(stderr, "Failed to create read struct for %s\n", path);
        return false;
    }
    png_infop info_ptr = png_create_info_struct(png_ptr);
    if (!info_ptr) {
        ...
        std::fclose(fp);
        fprintf(stderr, "Failed to create info struct for %s\n", path);
        return false;
    }
    if (setjmp(png_jmpbuf(png_ptr))) {
        png_destroy_read_struct(&png_ptr, &info_ptr, nullptr);
        std::fclose(fp);
        fprintf(stderr, "Failed to setjmp for %s\n", path);
        return false;
    }
    png_init_io(png_ptr, fp);
    ...
    if (color != PNG_COLOR_TYPE_GRAY || (depth != 8 && depth != 1)) {
        png_destroy_read_struct(&png_ptr, &info_ptr, nullptr);
        std::fclose(fp);
        fprintf(stderr, "Failed to get color type or bit depth for %s %d %d\n", path, color, depth);
        return false;
    }
...
auto SaveRGBAtoPNG(const char *path, int width, int height, const uint8_t *rgba, int stride)
    -> bool {
    FILE *fp = std::fopen(path, "wb");
    if (!fp) return false;
    png_structp png_ptr = png_create_write_struct(PNG_LIBPNG_VER_STRING, nullptr, nullptr, nullptr);
    ...
    std::vector<png_bytep> rows(static_cast<size_t>(height));
    for (int y = 0; y < height; ++y)
        rows[static_cast<size_t>(y)] = (png_bytep)(rgba + y * stride);
    png_write_image(png_ptr, rows.data());
    png_write_end(png_ptr, nullptr);
    png_destroy_write_struct(&png_ptr, &info_ptr);
    std::fclose(fp);
...
auto RGBAtoGray(const uint8_t *rgba, int width, int height, int stride, std::vector<uint8_t> &gray)
    -> void {
    ...
    for (int y = 0; y < height; ++y) {
        ...
        for (int x = 0; x < width; ++x) {
            ...
            gray[static_cast<size_t>(y) * width + x] = static_cast<uint8_t>((r + g + b) / 3);
        }
    }
}
